...
if "map" not in dmg.columns and "file" in dmg.columns and "map" in meta.columns:
    ...
    print("  map-Spalte per JOIN ergänzt")

# ── Hilfsfunktionen ───────────────────────────────────────────────────────────
def world_to_bin(series_x, series_y, mr, bins=BINS):
    """Konvertiert Weltkoordinaten direkt in Bin-Indizes (0..bins-1)."""
    sx, sy = float(mr["StartX"]), float(mr["StartY"])
    ex, ey = float(mr["EndX"]),   float(mr["EndY"])
    bx = ((series_x - sx) / (ex - sx) * bins).clip(0, bins-1).astype(int)
    by = ((series_y - sy) / (ey - sy) * bins).clip(0, bins-1).astype(int)
    return bx, by

def compute_grid(df, xcol, ycol, mr, bins=BINS):
    """Berechnet ein bins×bins Count-Grid aus Koordinatenspalten."""
    valid = df[[xcol, ycol]].dropna()
    valid = valid[(valid[xcol] != 0) | (valid[ycol] != 0)]
    if valid.empty:
        return np.zeros((bins, bins), dtype=np.float32)
    bx, by = world_to_bin(valid[xcol], valid[ycol], mr, bins)
    grid = np.zeros((bins, bins), dtype=np.float32)
    np.add.at(grid, (by, bx), 1)   # by=row, bx=col → (y, x) für imshow
    return grid
...
# ── Pro Karte berechnen ───────────────────────────────────────────────────────
print(f"\nBerechne {BINS}×{BINS} Heatmap-Bins pro Karte...\n")
...
for _, mr in map_data.iterrows():
    ...
    if os.path.exists(out_path):
        print(f"  ✓ {map_name} — bereits vorhanden, übersprungen")
        continue
    ...
    if not gr.empty and "nade_land_x" in gr.columns:
        ...
        if "nade" in gr.columns:
            for nade_type in gr["nade"].dropna().unique():
                sub = gr[gr["nade"] == nade_type]
...
print(f"\n✅ Fertig in {time.time()-total_start:.1f}s")
print(f"📁 Gespeichert in: {HEATMAP_DIR}")
print("\nVerfügbare Layer pro Karte:")
for _, mr in map_data.iterrows():
